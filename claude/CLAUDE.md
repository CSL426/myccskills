...
- **Division of labor between design skills**: `hallmark` = how it should look (taste rules, anti-patterns); `ui-component-libraries` = what to build with (library selection); `ui-ux-pro-max` = design data lookup (color palettes, font pairings, product-type UX guidelines, chart types) — consult it for concrete options/data, while `hallmark` still owns final taste decisions. Reserve `design-md` for when the user explicitly asks to imitate a specific brand's style; don't stack multiple taste skills on the same task.
