...
Core model: **the data repo is the source of truth.** `init` pulls live config INTO `~/ai-config/data`; `apply` pushes data repo config OUT to the live home dirs. Edit → `init` → commit/push in `data/` → other machine `sync` → `apply`.
