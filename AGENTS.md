...
這個 repo 集中管理多個 AI CLI(Claude Code / Codex / Antigravity)的設定,是 **skill、agent、command、MCP、rule 的管理中樞**。在這裡工作的主要任務通常是:評估新資源 → 決定安裝位置 → 透過 `ai-config.sh`(Linux / Unix)或 `ai-config.ps1`(Windows)呼叫共用 Python CLI 擴散到其他工具 → 提交版本控制。
...
兩個平台都需要 Python 3.11+。Windows 11 原生流程使用根目錄的 `ai-config.ps1` 薄 wrapper,不需要 WSL、Bash 或 `rsync`;完整平台說明與指令對照見 `README.md`。
...
- **改 `ai_config/**`、兩個 wrapper 或 `legacy/**` 後必跑測試**:`pytest tests/`;wrapper 另跑 `bash -n ai-config.sh legacy/ai-config.sh legacy/scripts/*.sh`。
